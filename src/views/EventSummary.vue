  ...
  <div class="p-6 bg-gray-100 min-h-screen">
    ...
    <div class="space-y-8">
      <div v-for="(event, eventId) in filteredEvents" :key="eventId" class="bg-white p-6 rounded-lg shadow-md border border-gray-300">
        <h2 class="text-2xl font-semibold mb-4 text-blue-600">{{ event.event_name }}</h2>
        ...
        <div class="grid grid-cols-2 gap-4 mb-4">
          <div class="bg-blue-50 p-4 rounded-lg shadow-inner">
            ...
            <p class="text-blue-800"><strong>Total Tickets Available:</strong> {{ event.total_tickets_available }}</p>
          </div>
          <div class="bg-green-50 p-4 rounded-lg shadow-inner">
            <p class="text-green-800"><strong>Valid Tickets:</strong> {{ event.total_valid_tickets }}</p>
            <p class="text-green-800"><strong>Redeemed Tickets:</strong> {{ event.total_redeemed_tickets }}</p>
          </div>
        </div>
        ...
        <div class="space-y-4">
          <div v-for="(category, index) in event.categories" :key="index" class="bg-gray-50 p-4 rounded-lg shadow-inner">
            <h4 class="text-lg font-semibold mb-2 text-green-600">{{ category.category_name }}</h4>
...
export default {
  ...
  mounted() {
    this.fetchEventsData(); // Fetch event data when component is mounted
  },
  computed: {
    filteredEvents() {
