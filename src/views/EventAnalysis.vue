<template>
  <div>
    ...
    <div class="p-6 max-w-4xl mx-auto bg-white shadow-xl rounded-lg">
      <h2
        class="text-4xl font-bold mb-8 text-center text-gradient bg-gradient-to-r from-green-400 via-yellow-400 to-red-400 text-transparent bg-clip-text"
      >
      ...
      <div
        class="flex flex-col md:flex-row mb-6 gap-4 items-center justify-between"
      >
        <!-- Event Search -->
      ...
      <!-- Loading State -->
      <div v-if="loading" class="text-center mt-8 text-gray-500 animate-pulse">
        Loading...
      </div>
      ...
      <div v-else>
        <div
          ...
          :key="eventId"
          class="mb-8 p-6 bg-gradient-to-r from-gray-50 to-white rounded-lg shadow-lg border-l-4 border-green-400"
        >
          ...
          <p class="text-gray-800 mb-2">
            Total Tickets Sold:
            <span class="font-bold">{{ event.total_tickets_sold }}</span>
          </p>
          <p class="text-gray-800 mb-4">
            Total Revenue:
            <span class="font-bold"
              >Ksh. {{ event.total_revenue.toFixed(2) }}</span
            >
          </p>
          ...
          <div class="mb-4">
            ...
            <ul class="ml-4 list-disc text-gray-800">
              ...
              >
                ...
                <span class="font-bold">{{ category.tickets_sold }}</span>
                tickets
              </li>
            </ul>
          </div>
    ...
    const fetchData = async () => {
      ...
      try {
        const response = await axios.get(
          '/api/analysis/get/tickets/byDuration',
          {
            params: {
