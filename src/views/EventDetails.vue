<template>
  <div class="min-h-screen bg-gray-100">
    <!-- Topbar -->
    <div
      class="bg-gradient-to-r from-red-400 to-green-900 text-white p-4 shadow-md flex items-center justify-between"
    >
      <!-- Logo -->
      <div class="ml-4">
        <img src="../assets/logo.jpg" alt="Logo" class="h-10 w-auto" />
      </div>

      <!-- Center Title -->
      <div class="text-center flex-1">
        <h1 class="text-2xl font-bold">Legacy Events</h1>
      </div>

      <div class="mr-4">
        <router-link
          to="/"
          class="text-white font-serif font-bold hover:underline text-lg"
      ...
      <div v-else-if="event" class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="space-y-6">
          ...
          <img
            :src="event.image_url"
            alt="Event Image"
            class="w-70 max-w-md h-80 object-cover rounded-lg shadow-md mx-auto"
          />
        ...
        <div class="space-y-6">
          <div class="bg-white p-6 rounded-lg shadow-md border border-gray-200">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div>
                <div
                  v-for="(category, index) in event.ticket_categories"
                  :key="index"
                  class="mb-4"
                >
                  ...
                  <p class="text-lg font-medium text-gray-700">
                    <strong>Price:</strong> Ksh. {{ category.price }}
                  </p>

                  <div class="flex items-center mt-2">
                    <button
                      @click="decreaseQuantity(index)"
                      class="bg-red-500 text-white px-4 py-2 rounded-md shadow-md hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-opacity-50"
                    >
                      -
                    </button>
                    ...
                    <button
                      ...
                      class="bg-green-500 text-white px-4 py-2 rounded-md shadow-md hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-opacity-50"
                    >
                      +
                    </button>
                  </div>
              ...
              <div class="flex flex-col justify-center items-center space-y-4">
                ...
                <div>
                  ...
                  />
                  <p class="text-gray-500 text-sm mt-2">
                    Please enter the email that will receive the ticket. Ensure
                    it is correct.
                  </p>
                ...
                <button
                  @click="buyTickets"
                  class="w-full bg-yellow-900 text-white py-2 px-4 rounded-md shadow-md hover:bg-yellow-700 focus:outline-none focus:ring-2 focus:ring-yellow-500 focus:ring-opacity-50 text-lg font-semibold mt-4"
                >
                  Buy Ticket(s)
                </button>
              </div>
...
export default {
  ...
  computed: {
    totalPrice() {
      return this.event.ticket_categories
        .reduce((total, category, index) => {
  ...
  methods: {
    ...
    increaseQuantity(index) {
      if (
        ...
        this.event.ticket_categories[index].quantity_available
      ) {
        this.quantities[index]++;
      }
    },
