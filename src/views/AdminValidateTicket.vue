    ...
    <div class="container mx-auto p-4">
      ...
      <div class="mb-4 flex flex-col sm:flex-row sm:gap-4 justify-center">
        ...
        <div class="relative w-full max-w-xs">
          ...
          />
          <div
            v-if="searchByEventName"
            class="absolute top-full left-0 w-full bg-white border border-gray-200 shadow-lg mt-2 p-2"
          >
            <p class="font-semibold">Validated: {{ validatedTicketsCount }}</p>
            <p class="font-semibold">
              Not Validated: {{ notValidatedTicketsCount }}
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="mb-4 flex flex-col sm:flex-row sm:gap-4 justify-center">
        <div class="w-full max-w-xs">
          <p class="font-semibold">
            Total Validated Tickets: {{ validatedTicketsCount }}
          </p>
        </div>
        <div class="w-full max-w-xs">
          <p class="font-semibold">
            Total Not Validated Tickets: {{ notValidatedTicketsCount }}
          </p>
        </div>
      </div>
      ...
      <div
        v-if="filteredTicketsByEventName.length > 0"
...
export default {
  ...
  setup() {
    ...
    const validatedTicketsCount = computed(() => {
      return filteredTicketsByEventName.value.filter(
        (ticket) => ticket.valid_status === 1
      ).length;
    });

    const notValidatedTicketsCount = computed(() => {
      return filteredTicketsByEventName.value.filter(
        (ticket) => ticket.valid_status === 0
      ).length;
    });
    ...
    const validateTicket = async (ticketId) => {
      try {
        await axios.post(
          `/api/update/ticket/status/${ticketId}`,
          {},
          {
            headers: {
              Authorization: `Bearer ${auth.getToken()}`,
