<template>
  ...
  >
    <div
      id="ticket-container"
      class="bg-gradient-to-br from-white to-gray-100 p-4 rounded-lg shadow-2xl max-w-xs w-full relative border-2 border-gray-300"
    >
      ...
      <!-- Ticket details -->
      <h2 class="text-2xl font-bold mb-4 text-center text-gray-700 font-sans">
        Ticket Details
      </h2>
      <div v-if="ticket" class="text-center space-y-2">
        ...
        <p class="text-rose-900 font-medium text-base">
          <strong>Category:</strong> {{ ticket.category_name }}
        </p>
        <p class="text-gray-700 font-medium text-base">
          <strong>Ticket Number:</strong> {{ ticket.ticket_id }}
        </p>
        ...
        <p class="text-gray-700 text-base">
          ...
          <span
            :class="{
              'text-green-500 font-bold': ticket.valid_status === '1',
              'text-red-500 font-bold': ticket.valid_status === '0',
            }"
          >
        ...
        <div v-if="ticket.qr_code" class="text-center mt-4">
          <img
            ...
            ref="qrCodeImage"
            alt="QR Code"
            class="mx-auto rounded-lg shadow-lg w-24 h-24"
          />
          <p class="text-sm text-gray-500 mt-2">
            Visit:
            <a
              href="http://www.legacyevents.com"
              target="_blank"
              class="underline text-blue-500"
              >www.legacyevents.com</a
            >
          </p>
        </div>
        <p v-else class="mt-4 text-red-500 text-center">
      ...
      <div v-if="!isDownloading" class="mt-4 flex justify-center space-x-4">
        <button
          ...
          class="bg-green-500 hover:bg-green-600 text-white py-2 px-4 rounded shadow-md font-medium"
        >
          Download Ticket
        </button>
        <button
          @click="$emit('close')"
          class="bg-red-500 hover:bg-red-600 text-white py-2 px-4 rounded shadow-md font-medium"
        >
          Close
        </button>
      </div>
